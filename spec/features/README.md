...
| [Id-scoped subcollection definitions and persisted placeholder names](scoped-subcollection-definitions/README.md) | Draft | A subcollection definition is stored under the id position of its parent collection, not just under its name, so `ext/datatug/projects` and `ext/sneat/projects` are independent definitions and each placeholder name is persisted once. |
